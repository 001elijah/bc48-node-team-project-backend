...
const initializBackgrounds = async () => {
  try {
    ...
    for (const thumbnailImage of thumbnailImages) {
      ...
      const images = {
        mobileUrl_1x: getImageUrl(resources, filename, 'background/mobile/x1'),
        mobileUrl_2x: getImageUrl(resources, filename, 'background/mobile/x2'),
        tabletUrl_1x: getImageUrl(resources, filename, 'background/tablet/x1'),
        tabletUrl_2x: getImageUrl(resources, filename, 'background/tablet/x2'),
        desktopUrl_1x: getImageUrl(resources, filename, 'background/desktop/x1'),
        desktopUrl_2x: getImageUrl(resources, filename, 'background/desktop/x2'),
...
const getImageUrl = (resources, filename, folder) => {
  const image = resources.find(
    (resource) => resource.folder === folder && resource.filename === filename
  );
